...
***** TODO configure the org-roam buffer display
...
#+begin_src emacs-lisp :tangle init.el
  (use-package citar
    :after all-the-icons
    :hook (LaTeX-mode . (add-to-list 'completion-at-point-functions 'citar-capf))
    :config
...
*** TODO this isn't working so well with haskell-mode
...
- add ~citar-capf~ to latex buffers using cape!
...
- Accents are interfering with apostrophes!
...
- [[https://emacs.stackexchange.com/questions/45629/template-for-new-file][Template for new file - Emacs Stack Exchange]]
...
**** TODO setup built in package ~autoinsert~ for file templates via yasnippet.
